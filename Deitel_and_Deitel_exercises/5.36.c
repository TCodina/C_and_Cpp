/*
Exercise 5.36 (Towers of Hanoi)
Every budding computer scientist must grapple with certain classic
problems, and the Towers of Hanoi is one of the most famous of these. Legend has
...
#include <stdio.h>
#define DISKS 5
#define INITIAL_PEG 1
#define TEMPORARY_PEG 2
#define DESTINATION_PEG 3

void HanoiTowerAlgorithm( int numberOfDisks, int initialPeg, int temporaryPeg, int destinationPeg );

int main( void )
{

    printf( "To solve the Hanoi tower problem of moving %d disks"
    "\nfrom peg %d to peg %d using peg %d as a temporary peg,"
    "\nfollow the algorithm:\n\n", DISKS, INITIAL_PEG, DESTINATION_PEG, TEMPORARY_PEG );

    HanoiTowerAlgorithm( DISKS, INITIAL_PEG, TEMPORARY_PEG, DESTINATION_PEG );
...
void HanoiTowerAlgorithm( int numberOfDisks, int initialPeg, int temporaryPeg, int destinationPeg )
{
    // base case
    if ( numberOfDisks == 1 ) {
        printf( "%d -> %d\n", initialPeg, destinationPeg );
    }
    // recursive step until reach base case
    else {
        HanoiTowerAlgorithm( numberOfDisks - 1, initialPeg, destinationPeg, temporaryPeg );
        printf( "%d -> %d\n", initialPeg, destinationPeg );
        HanoiTowerAlgorithm( numberOfDisks - 1, temporaryPeg, initialPeg, destinationPeg );
    }
}
